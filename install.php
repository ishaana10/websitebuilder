<?php
/**
 * Nuvis WebDesign X Proprietary Site Builder - Automated Installer and DB Seeder
 */
...
function run_installation_process($pdo, $output_callback = null) {
    ...
    if ($res_tenants['tenant_count'] == 0) {
        ...
        $insert_tenant->execute([1, 'Nuvis Global Headquarters', 'hq', 'hq.nuvis-webdesign-x.io', 'agency', 'active']);
    ...
    if ($res['admin_count'] == 0) {
        ...
        $admin_user = 'admin';
        $admin_email = 'admin@nuvis-webdesign-x.io';
...
<div data-component-instance="contact" class="bg-slate-900 py-16 px-6 text-slate-300">
    <div class="max-w-lg mx-auto bg-slate-950 p-8 rounded-2xl border border-slate-800">
        ...
        <form onsubmit="submitNuvisWebDesignXForm(event, this)">
            <div class="mb-4">
...
<section class="py-12 bg-white px-6 md:px-12 relative z-20 -mt-8 max-w-6xl mx-auto rounded-2xl shadow-xl border border-slate-100" data-component="fullwidth_raw_html">
    ...
    <form class="grid grid-cols-1 md:grid-cols-4 gap-4" onsubmit="event.preventDefault(); window.submitNuvisWebDesignXForm(this);">
        <div class="nuvis-webdesign-x-form-status hidden col-span-1 md:col-span-4 p-3 rounded text-xs font-bold text-center"></div>
...
<section id="contact" class="py-20 bg-slate-50 px-6 md:px-12" data-component="fullwidth_raw_html">
    <div class="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        ...
        <div class="lg:col-span-6 bg-white p-8 rounded-2xl shadow-xl border border-slate-100">
            ...
            <form onsubmit="event.preventDefault(); window.submitNuvisWebDesignXForm(this);" class="space-y-4">
...
if ($is_cli) {
    ...
    echo "=== Nuvis WebDesign X Automated System Installer ===\n\n";
    ...
    try {
        ...
        if (function_exists('write_system_log')) {
            write_system_log('info', 'Nuvis WebDesign X installation successfully completed via CLI.');
        }

        echo "\n=== Nuvis WebDesign X System Successfully Installed! ===\n";
        exit(0);
    } catch (Exception $e) {
        echo "\n❌ INSTALLATION FAILED!\n";
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        try {
            ...
            if (function_exists('write_system_log')) {
                write_system_log('info', 'Nuvis WebDesign X installation started via guided web installer.');
                ...
                write_system_log('info', 'Nuvis WebDesign X installation completed successfully.');
            }
    ...
    <head>
        ...
        <title>Nuvis WebDesign X - Guided Interactive Setup</title>
    ...
    <body class="h-full flex flex-col justify-between text-slate-100">
        ...
        <header class="bg-slate-950 border-b border-slate-800 py-5 px-6">
            <div class="max-w-5xl mx-auto flex items-center justify-between">
                ...
                <div class="text-xs text-slate-400 font-medium bg-slate-900 border border-slate-800 px-3 py-1.5 rounded-full">
                    <span class="w-2 h-2 inline-block bg-emerald-400 rounded-full mr-1.5 animate-pulse"></span> Proprietary Installer
                </div>
            </div>
        </header>
        ...
        <main class="flex-1 py-12 px-4 max-w-5xl mx-auto w-full">
            <?php if ($already_installed && empty($success_msg)): ?>
                <div class="bg-slate-950 border border-slate-800 p-8 rounded-2xl shadow-2xl text-center space-y-6">
                    ...
                    <h2 class="text-2xl font-extrabold text-white">Nuvis WebDesign X is Already Installed!</h2>
            ...
            <?php elseif (!empty($success_msg)): ?>
                ...
                <div class="bg-slate-950 border border-slate-800 rounded-2xl shadow-2xl p-8 space-y-8">
                    <div class="text-center space-y-3">
                        ...
                        <p class="text-emerald-400 text-sm font-semibold">Nuvis WebDesign X is fully configured and ready for production.</p>
                    </div>
        ...
        <footer class="bg-slate-950 border-t border-slate-800 py-6 px-6 text-center text-xs text-slate-500">
            <div class="max-w-5xl mx-auto">
                <p>Nuvis WebDesign X Installer &copy; <?php echo date('Y'); ?>. All rights reserved.</p>
            </div>
        </footer>
    </body>
    </html>
